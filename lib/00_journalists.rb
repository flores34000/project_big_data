...
puts "\033[31m"+"Quel est le handle le plus court de cette liste ? 🤔"+"\033[0m"

puts ""

max = handle.max {|a,b| b.length <=> a.length}
puts max

puts ""
puts ""

puts "\033[31m"+"Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)🤔"+"\033[0m"

puts ""

cinq_caractere = handle.select { |index| index.length == 6 }
puts cinq_caractere

puts ""

cinq_caractere_nombre = cinq_caractere.length

puts "il y a #{cinq_caractere_nombre} handle qui ont cinq caractere"
...
puts ""
puts ""
puts "\033[31m"+"Trie la liste de handle par ordre alphabétique." +"\033[0m"

puts ""
alphabet = handle.sort{|a,b| a.downcase <=> b.downcase}
puts alphabet

puts ""
puts ""

puts "\033[31m"+"Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)🤔" +"\033[0m"

puts ""

taille_handle = handle.sort {|a,b| a.length <=> b.length}
puts taille_handle

puts ""
puts ""

puts "\033[31m"+"Quelle est la position dans l'array de la personne @epenser ?🤔" +"\033[0m"
puts ""
